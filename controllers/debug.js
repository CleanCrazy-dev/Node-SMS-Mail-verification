...
debugController.populatedatabase = function (req, res) {
  winston.debug('Entering populatedatabase...');
      ...
      function (done) {
        ...
        for (var i = 0; i < 11; i++) {
          ...
          var user = {
            ...
            phone: '+1' + chance.phone({ country: 'us', formatted: false }),
            officePhone:
...
debugController.sendmail = function (req, res) {
    ...
    render: function (view, locals) {
      return new Promise(function (resolve, reject) {
        if (!global.Handlebars)
          return reject(new Error('Could not load global.Handlebars'));
        templateSchema.findOne({ name: view }, function (err, template) {
          if (err) return reject(err);
          if (!template) return reject(new Error('Invalid Template'));
          var html = global.Handlebars.compile(template.data['gjs-fullHtml'])(
            locals
          );
          console.log(html);
  ...
  email
    .render('ticket-updated', {
      base_url: global.HUB_BASEURL,
      ticket: ticket,
    })
    .then(function (html) {
      var mailOptions = {
        to: to,
        subject: 'Tridacom Hub Test Email [Debugger]',
        html: html,
...
debugController.uploadPlugin = function (req, res) {
  ...
  busboy.on('finish', function () {
    ...
    fs.createReadStream(object.filePath).pipe(
      unzipper.Extract({ path: path.join(__dirname, '../../plugins') })
    );
