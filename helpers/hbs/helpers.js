...
var helpers = {
  ...
  isNotTrue: function (obj, options) {
    if (
      obj === true ||
      (typeof obj.toLowerCase === 'function' && obj.toLowerCase() === 'true')
    )
      return options.inverse(this);
  ...
  hasPermOverRole: function (ownerRole, userRole, perm, options) {
    if (
      _.isUndefined(ownerRole) ||
      _.isUndefined(userRole) ||
      _.isUndefined(perm)
    )
      return options.inverse(this);
    if (
      ...
      throw new Error(
        'Invalid Type sent to hasPermOverRole. Should be role object'
      );
    }

    var p = require('../../permissions');
    if (!p.canThis(userRole, perm)) return options.inverse(this);
    if (ownerRole._id.toString() === userRole._id.toString())
      return options.fn(this);
    ...
    if (hasHierarchyEnabled && p.hasPermOverRole(ownerRole._id, userRole._id))
      return options.fn(this);
