        ...
        roleSchema.getRoleByName('User', function (err, role) {
          ...
          roleSchema.create(
            ...
            function (err, userRole) {
              if (err) return done(err);
              SettingsSchema.getSetting('role:user:default', function (
                err,
                roleUserDefault
              ) {
                if (err) return done(err);
    ...
    if (!setting) {
      ...
      ticketTypeSchema.getTypes(function (err, types) {
        ...
        if (!_.isObject(type) || _.isUndefined(type._id))
          return callback('Invalid Type. Skipping.');
...
function mailTemplates(callback) {
  ...
  async.parallel(
    [
      function (done) {
        templateSchema.findOne({ name: newTicket.name }, function (
          err,
          templates
        ) {
          if (err) return done(err);
      ...
      function (done) {
        templateSchema.findOne({ name: passwordReset.name }, function (
          err,
          templates
        ) {
          if (err) return done(err);
...
settingsDefaults.init = function (callback) {
  ...
  async.series(
    [
      ...
      winston.debug('Finished Checking Settings...'),
    ],
    function (err) {
