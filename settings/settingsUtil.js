...
util.getSettings = function (callback) {
  settingSchema.getSettings(function (err, settings) {
    ...
    s.siteTitle = parseSetting(settings, 'gen:sitetitle', 'Tridacom Hub');
    ...
    s.mailerCheckPolling = parseSetting(
      settings,
      'mailer:check:polling',
      600000
    );
    ...
    s.elasticSearchConfigured = {
      value:
        s.elasticSearchEnabled.value !== false &&
        !_.isEmpty(s.elasticSearchHost.value),
    };
    ...
    s.allowAgentUserTickets = parseSetting(
      settings,
      'allowAgentUserTickets:enable',
      false
    );
