...
events.onSetUserIdle = function (socket) {
  socket.on('$trihub:setUserIdle', function () {
...
events.onSetUserActive = function (socket) {
  socket.on('$trihub:setUserActive', function () {
    var user = socket.request.user;
    if (sharedVars.idleUsers.hasOwnProperty(user.username.toLowerCase())) {
...
function updateOnlineBubbles() {
  ...
  utils.sendToAllConnectedClients(io, '$trihub:chat:updateOnlineBubbles', {
...
events.updateOnlineBubbles = function (socket) {
  socket.on('$trihub:chat:updateOnlineBubbles', function () {
    updateOnlineBubbles();
  });
};
